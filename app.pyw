...
class AppConfig(engine.AppConfig):
    ...
    def load(self) -> None:
        ...
        self.geometry = normalized_geometry(self.geometry)
...
class WidgetApp:
    ...
    def _send_worker(self, text: str) -> None:
        ...
        All of the sequencing (synthesize, verify the audio, activate WeChat,
        enter recording mode, play through VB-CABLE, click send, and always
        leave recording mode) lives in engine.VoiceEngine so that the CLI path
        and the GUI path cannot drift apart.
        """
        try:
